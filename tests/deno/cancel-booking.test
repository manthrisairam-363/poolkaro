...
async function setWallet(uid: string, bal: number) {
  await db.from('wallets').update({ balance: bal }).eq('user_id', uid)
}
...
Deno.test('CB_T07 - Rider wallet credited 200 paise', async () => {
  ...
  try {
    const booking = await bookRide(ride.id, riderId)
    const beforeCancel = await getWallet(riderId)
    await cancelBooking(booking.booking.id, riderId)
    assertEquals(await getWallet(riderId), beforeCancel + 200)
  } finally { await cleanup(ride.id) }
})
...
Deno.test('CB_T08 - Driver wallet credited 200 paise', async () => {
  ...
  try {
    const booking = await bookRide(ride.id, riderId)
    const beforeCancel = await getWallet(driverId)
    await cancelBooking(booking.booking.id, riderId)
    assertEquals(await getWallet(driverId), beforeCancel + 200)
  } finally { await cleanup(ride.id) }
})
...
Deno.test('CB_T13 - is_verified set false on 5th+ cancellation', async () => {
  ...
  try {
    const booking = await bookRide(ride.id, riderId)
    if (!booking?.booking?.id) throw new Error('Booking failed: ' + JSON.stringify(booking))
    bookingId = booking.booking.id as string
    await cancelBooking(bookingId as string, riderId)
...
Deno.test('CB_T17 - Wallet balances unchanged when cancellation fails', async () => {
  ...
  const before = await getWallet(riderId)
  await cancelBooking('00000000-0000-0000-0000-000000000000', riderId)
  assertEquals(await getWallet(riderId), before)
})
