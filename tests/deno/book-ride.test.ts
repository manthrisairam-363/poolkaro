...
Deno.test('T08 - Rider wallet deducted 200 paise per seat', async () => {
  ...
  try {
    const { data, error } = await book(ride.id, riderId, 1)
    // Verify booking succeeded first
    if (error || !data?.success) {
      throw new Error(`Booking failed: ${error?.message || JSON.stringify(data)}`)
    }
    // Verify deduction via transaction record (more reliable than balance)
...
Deno.test('T09 - Driver wallet deducted 200 paise per seat', async () => {
  ...
  try {
    ...
    const { data: txn } = await db.from('wallet_transactions')
      .select('amount').eq('user_id', driverId).eq('type', 'posting_fee')
      .order('created_at', { ascending: false }).limit(1).single()
    assertEquals(txn?.amount, -200)
  } finally { await cleanup(ride.id) }
})
