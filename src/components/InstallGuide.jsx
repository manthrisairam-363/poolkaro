...
export default function InstallGuide() {
  ...
  return (
    <div style={{ position: 'fixed', left: 0, right: 0, bottom: 'calc(env(safe-area-inset-bottom) + 74px)', zIndex: 900, padding: '0 12px', pointerEvents: 'none' }}>
      <div style={{ maxWidth: 480, margin: '0 auto', background: 'rgba(17,17,17,0.55)', borderRadius: 14, boxShadow: '0 8px 30px rgba(0,0,0,0.4)', border: '1px solid #333', overflow: 'hidden', pointerEvents: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px' }}>
          ...
          <button onClick={() => setExpanded(v => !v)} style={{ padding: '7px 12px', background: '#facc15', color: '#111', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 800, cursor: 'pointer', flexShrink: 0 }}>{expanded ? 'Hide' : 'How?'}</button>
        ...
        {expanded && (
          <div style={{ padding: '0 14px 14px' }}>
            <div style={{ fontSize: 11, color: '#facc15', fontWeight: 700, marginBottom: 8 }}>{isIOS ? '🍎 On your iPhone (Safari)' : '🤖 On your Android (Chrome)'}</div>
            ...
            {isIOS && <div style={{ fontSize: 10.5, color: '#777', marginTop: 8, lineHeight: 1.5 }}>Note: on iPhone this only works in <b style={{ color: '#aaa' }}>Safari</b>, not Chrome. Notifications work once added to the home screen.</div>}
          </div>
        )}
      </div>
    </div>
